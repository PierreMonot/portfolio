...
```
Faire 2 pour choisir un mot de passe complexe
```
...
```
Répondre y sur les deux questions
```
...
mysql> quit; 
```
6. Import du schéma de la base de données
```
zcat /usr/share/zabbix/sql-scripts/mysql/server.sql.gz | mysql --default-character-set=utf8mb4 -uzabbix -p zabbix
```
7. Désactivation du log_bin_trust_function_creators
```
mysql -uroot -p
mysql> set global log_bin_trust_function_creators = 0;
mysql> quit;
```
8. Modifier le fichier Zabbix /etc/zabbix/zabbix_server.conf
```
DBPassword=InsérerMotDePasse123!
```
9. Redémarrage des services Apache + activation au démarrage de la machine
```
